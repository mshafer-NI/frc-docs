...
For the roboRIO 2, image the micro SD card
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

.. important:: The first time a roboRIO 2 is imaged (or if the microSD card is replaced with a new card), it must be imaged using :doc:`roboRIO 2.0 microSD card imaging process</docs/software/roborio-info/roborio2-imaging>`. After that, the imaging tool is only used to set the team number.
